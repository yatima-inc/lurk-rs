...
impl<F: LurkField> HashStub<F> {
  pub fn car_cdr(
    ...
    match self {
      ...
      Self::Blank => {
        unreachable!("Blank HashStub should be used only in blank circuits.")
      },
...
impl<F: LurkField> HashWitness<F> {
  pub fn new_from_stub(stub: HashStub<F>) -> Self {
    Self { slots: [(ConsName::NeverUsed, stub); MAX_CONSES_PER_REDUCTION] }
  }

  pub fn new_dummy() -> Self { Self::new_from_stub(HashStub::Dummy) }

  pub fn new_blank() -> Self { Self::new_from_stub(HashStub::Blank) }
  ...
  pub fn conses_used(&self) -> Vec<HashStub<F>> {
    self.all_conses().into_iter().filter(|c| !c.is_dummy()).collect()
  }
  ...
  pub fn total_conses(&self) -> usize { self.all_conses().len() }
}
