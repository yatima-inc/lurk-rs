  ...
  where D: serde::Deserializer<'de> {
    ...
    let (tag, dig) = F::from_cid(cid).ok_or_else(|| {
      D::Error::custom(format!(
        "expected ScalarPtr encoded as Cid, got {}",
        cid
      ))
    })?;
...
impl<F: LurkField> Store<F> {
  ...
  pub fn car(&self, expr: &Ptr<F>) -> Ptr<F> { self.car_cdr(expr).0 }

  pub fn cdr(&self, expr: &Ptr<F>) -> Ptr<F> { self.car_cdr(expr).1 }
  ...
  pub fn intern_scalar_cont_ptr(
    ...
    match (tag, cont) {
      (ContTag::Outermost, Some(Outermost)) => {
        Some(self.intern_cont_outermost())
      },
      (ContTag::Call, Some(Call { unevaled_arg, saved_env, continuation })) => {
      ...
      (
        ContTag::Binop2,
        Some(Binop2 { operator, evaled_arg, continuation }),
      ) => {
        let arg = self.intern_scalar_ptr(*evaled_arg, scalar_store)?;
        let cont = self.intern_scalar_cont_ptr(*continuation, scalar_store)?;
        Some(self.intern_cont_binop2(*operator, arg, cont))
      },
      (ContTag::If, Some(If { unevaled_args, continuation })) => {
        let args = self.intern_scalar_ptr(*unevaled_args, scalar_store)?;
        let cont = self.intern_scalar_cont_ptr(*continuation, scalar_store)?;
        Some(self.intern_cont_if(args, cont))
      },
      (ContTag::Let, Some(Let { var, body, saved_env, continuation })) => {
      ...
      (
        ContTag::LetRec,
        Some(LetRec { var, body, saved_env, continuation }),
      ) => {
        let var = self.intern_scalar_ptr(*var, scalar_store)?;
        let body = self.intern_scalar_ptr(*body, scalar_store)?;
        let env = self.intern_scalar_ptr(*saved_env, scalar_store)?;
  ...
  pub fn fetch_cont(&self, ptr: &ContPtr<F>) -> Option<Continuation<F>> {
    ...
    match ptr.0 {
      ...
      LetRec => {
        self.let_rec_store.get_index(ptr.1.idx()).map(|(a, b, c, d)| {
          Continuation::LetRec {
            var: *a,
            body: *b,
            saved_env: *c,
    ...
    match ptr.0 {
      ...
      Tag::Cons => match self.fetch(ptr) {
        ...
        Some(Expression::Opaque(_)) => {
          Err("cannot destructure opaque Cons".into())
        },
        _ => unreachable!(),
      },
      Tag::Str => {
  ...
  pub fn car_cdr(&self, ptr: &Ptr<F>) -> (Ptr<F>, Ptr<F>) {
    ...
    match ptr.0 {
      ...
      _ => {
        // FIXME: Don't panic. This can happen at runtime in a valid Lurk
        // program, so it should result in an explicit error.
