...
> {
    ...
    )?;
  {
    ...
    enforce_implication(
      &mut cs.namespace(|| {
        "(expr.tag == thunk_tag) implies (expr_thunk_hash == expr.hash)"
      }),
      &expr_is_a_thunk,
      &expr_is_the_thunk,
    )?;
...
> {
  ...
  let otherwise_and_cons =
    and!(cs, &otherwise_and_binding_not_nil, &var_or_rec_binding_is_cons)?;
  ...
  let val2_is_fun =
    alloc_equal(cs.namespace(|| "val2_is_fun"), val2.tag(), &g.fun_tag)?;
  ...
  let otherwise_neither =
    and!(cs, &var_or_rec_binding_is_neither, &otherwise_and_binding_not_nil)?;
  ...
  let cont_not_lookup_sym =
    and!(cs, &Boolean::not(&cont_is_lookup), &otherwise_and_sym)?;

  let cont_is_lookup_cons =
    and!(cs, &cont_is_lookup, &otherwise_and_v2_not_expr)?;
  let cont_not_lookup_cons =
    and!(cs, &Boolean::not(&cont_is_lookup), &otherwise_and_v2_not_expr)?;
  ...
  let extended_env_not_dummy =
    and!(cs, &val2_is_fun, not_dummy, &v2_is_expr_real)?;
  ...
  let smaller_rec_env_not_dummy =
    and!(cs, &smaller_rec_env_not_nil, not_dummy, &otherwise_and_v2_not_expr)?;
  ...
  let env_to_use =
    ifx_t!(cs, &smaller_rec_env_is_nil, &smaller_env, &with_smaller_rec_env)?;
...
> {
  ...
  let arg1_tag_is_num =
    alloc_equal(&mut cs.namespace(|| "tag_is_num"), arg1.tag(), &g.num_tag)?;
...
> {
  ...
  let (computed_cont_hash, cont_components) =
    ContPtr::allocate_maybe_dummy_components(
...
> {
  ...
  let (_continuation_tag, continuation_components) =
    ContPtr::allocate_maybe_dummy_components(
      &mut cs.namespace(|| "allocate_continuation_components"),
      witness.as_ref().and_then(|w| w.apply_continuation_cont.as_ref()),
      store,
    )?;
  ...
  let (unop_val, unop_continuation) = {
    ...
    let res_cdr =
      ifx_t!(cs, &result_is_empty_str, &g.empty_str_ptr, &allocated_cdr)?;
    ...
    let res = multi_case(
      ...
      &[
        &[
          ...
          CaseClause { key: Op1::Char.as_field(), value: c.tag() },
        ...
        &[
          ...
          CaseClause { key: Op1::Char.as_field(), value: c.hash() },
  ...
  let (the_expr, the_env, the_cont) = {
    ...
    let fun_body_cons_not_dummy =
      and!(cs, &body_t_is_cons, &args_is_dummy, &cont_is_call0, not_dummy)?;
  ...
  let (the_expr, the_env, the_cont) = {
...

    {
      ...
      let extend_not_dummy =
        and!(cs, &cont_is_call2_and_not_dummy, &Boolean::not(&args_is_dummy))?;
  ...
  let (the_expr, the_env, the_cont, make_thunk_num) = {
    ...
    let tags_equal =
      alloc_equal(&mut cs.namespace(|| "tags equal"), arg1.tag(), arg2.tag())?;
    ...
    let is_cons_or_hide = constraints::or(
      &mut cs.namespace(|| "is cons or hide"),
      &is_cons,
      &is_hide,
    )?;
    ...
    let double_a_bits =
      double_a.to_bits_le_strict(&mut cs.namespace(|| "double a lsb")).unwrap();
    let lsb_2a = double_a_bits.get(0);

    let double_b = constraints::add(&mut cs.namespace(|| "double b"), b, b)?;
    let double_b_bits =
      double_b.to_bits_le_strict(&mut cs.namespace(|| "double b lsb")).unwrap();
    let lsb_2b = double_b_bits.get(0);

    let diff_is_zero =
      alloc_is_zero(&mut cs.namespace(|| "diff is zero"), &diff)?;
    let double_diff =
      constraints::add(&mut cs.namespace(|| "double diff"), &diff, &diff)?;
    ...
    // least significant bit) is odd after doubling, meaning that the field
    // element (after doubling) is larger than the underlying prime p that
    // defines the field, then a modular reduction must have been carried
    // out, changing the parity that should be even (since we multiplied by
    // 2) to odd. In other words, we define negative numbers to be those
    // field elements that are larger than p/2.
    ...
    let op2_not_both_num_and_not_cons_or_strcons_or_hide_or_equal_or_num_equal =
      Boolean::and(
        &mut cs.namespace(|| {
          "not both num and not cons or strcons or hide or equal or num_equal"
        }),
...
//
// FIXME: no range-checking is performed, so the result could be an invalid
// Char. lurk-rs won't actually create such proofs because the out-of-range
// input will lead to an error when evaluating, but malicious input could still
// lead to such a proof. This would violate the principle that Lurk programs
// over valid input data should always return valid output data.
fn to_char<F: LurkField>(
  x: &AllocatedPtr<F>,
  g: &GlobalAllocations<F>,
) -> AllocatedPtr<F> {
...
fn car_cdr_named<F: LurkField, CS: ConstraintSystem<F>>(
  ...
  let maybe_cons_is_nil = maybe_cons
    .alloc_equal(&mut cs.namespace(|| "maybe_cons_is_nil"), &g.nil_ptr)?;
