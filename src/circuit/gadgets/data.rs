...
impl<F: LurkField> ContPtr<F> {
  ...
  fn allocate_components<CS: ConstraintSystem<F>>(
    ...
    let component_frs =
      store.get_hash_components_cont(self).expect("missing hash components");
  ...
  fn allocate_dummy_components<CS: ConstraintSystem<F>>(
    ...
    // We need to create these constraints, but eventually we can avoid doing
    // any calculation. We just need a precomputed dummy witness.
